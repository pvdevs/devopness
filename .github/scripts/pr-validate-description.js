...
/**
 * Validates the "Description of Changes" section
 * @param {PRTemplateSection} descriptionOfChanges - Processed markdown content
 */
function validateDescriptionOfChanges(descriptionOfChanges) {
  ...
  const changes = descriptionOfChanges.bodies
    ...
    .filter((item) => {
      ...
      // Get content without comments and checkboxes
      let content = item.raw
        .replace(/<!--[\s\S]*?-->/g, "") // Remove HTML comments
        .replace(/^\s*-\s*\[\s*[\sxX]?\s*\]\s*/, "") // Remove checkbox
        .trim();

      // Check if content only contains a placeholder (text within angle brackets)
      const placeholderOnly = /^\s*<[^>]+>\s*$/.test(content);

      // Return true only if there's content and it's not just a placeholder
      return content.length > 0 && !placeholderOnly;
    });

  if (!changes.length) {
    fail(
      "Description of Changes",
      "Pull requests must include at least one specific change in the 'Description of changes' field. Template placeholders alone are not valid descriptions.",
    );
    return;
  }
  ...
  const meaningfulChanges = changes.filter((item) => {
    // Clean the content for length checking
    let content = item.raw
      .replace(/<!--[\s\S]*?-->/g, "") // Remove HTML comments
      .replace(/^\s*-\s*\[\s*[\sxX]?\s*\]\s*/, "") // Remove checkbox
  ...
  if (!meaningfulChanges.length) {
    fail(
      "Description of Changes",
      "Description items must contain meaningful content beyond placeholders and checkboxes.",
    );
    return;
  }
  ...
  pass(
    ...
    meaningfulChanges
      .map((item) => {
        // For display purposes, clean up extra whitespace but keep the original content
